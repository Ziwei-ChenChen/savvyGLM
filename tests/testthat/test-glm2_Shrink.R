...
test_that("Logistic regression with intercept", {
  ...
  fit1 <- suppressWarnings(glm2_Shrink(y1 ~ ., family = binomial(link = "logit"), data = data1,  model_class = "St"))
...
test_that("Logistic regression without intercept", {
  ...
  fit2 <- suppressWarnings(glm2_Shrink(y2 ~ . - 1, family = binomial(link = "logit"), data = data2))
...
test_that("Gaussian family with intercept", {
  ...
  fit3 <- suppressWarnings(glm2_Shrink(y3 ~ ., family = gaussian(), data = data3, model_class = "GSR"))
...
test_that("Poisson family with intercept", {
  ...
  fit4 <- suppressWarnings(glm2_Shrink(y4 ~ ., family = poisson(), data = data4))
...
test_that("Gaussian family without intercept", {
  ...
  fit5 <- suppressWarnings(glm2_Shrink(y5 ~ . - 1, family = gaussian(), data = data5))
...
test_that("Handling missing data", {
  ...
  data <- data.frame(y, x)
  fit <- suppressWarnings(glm2_Shrink(y ~ ., family = binomial(link = "logit"), data = data, na.action = na.omit))
...
test_that("Different control parameters", {
  ...
  y <- rbinom(n, 1, prob = 0.5)
  data <- data.frame(y, x)
  fit <- suppressWarnings(glm2_Shrink(y ~ ., family = binomial(link = "logit"), data = data,
...
test_that("Different link functions for Gaussian family", {
  ...
  fit_identity <- suppressWarnings(glm2_Shrink(y ~ ., family = gaussian(link = "identity"), data = data.frame(y, x)))
  ...
  fit_log <- suppressWarnings(try(glm2_Shrink(y ~ ., family = gaussian(link = "log"),
                                              data = data.frame(y, x)), silent = TRUE))
  if (inherits(fit_log, "try-error")) {
    expect_error(glm2_Shrink(y ~ ., family = gaussian(link = "log"), data = data.frame(y, x), model_class = c("SR","GSR", "St")),
  ...
  fit_inverse <- suppressWarnings(glm2_Shrink(y ~ ., family = gaussian(link = "inverse"), data = data.frame(y, x)))
...
test_that("Rank-deficient matrix handling", {
  ...
  y <- rbinom(n, 1, prob = 0.5)
  x[, 2] <- x[, 1]
  fit <- suppressWarnings(glm2_Shrink(y ~ ., family = binomial(link = "logit"), data = data.frame(y, x)))
...
test_that("Convergence with different starting values", {
  ...
  fit <- suppressWarnings(glm2_Shrink(y ~ ., family = binomial(link = "logit"), data = data, start = start_values))
...
test_that("Family argument as a string", {
  ...
  x <- matrix(rnorm(n * p), n, p)
  y <- rbinom(n, 1, prob = 0.5)
  fit <- suppressWarnings(glm2_Shrink(y ~ ., family = "binomial", data = data.frame(y, x)))
...
test_that("Invalid family argument", {
  expect_error(glm2_Shrink(y ~ ., family = "invalid_family", data = data.frame(y = rbinom(100, 1, prob = 0.5), x = matrix(rnorm(100 * 5), 100, 5))),
...
test_that("Using model.frame method", {
  ...
  x <- matrix(rnorm(n * p), n, p)
  y <- rbinom(n, 1, prob = 0.5)
  mf <- glm2_Shrink(y ~ ., family = binomial(link = "logit"), data = data.frame(y, x), method = "model.frame")
...
test_that("Handling weights", {
  ...
  y <- rbinom(n, 1, prob = 0.5)
  weights <- runif(n)
  fit <- suppressWarnings(glm2_Shrink(y ~ ., family = binomial(link = "logit"), data = data.frame(y, x), weights = weights))
...
test_that("Handling offsets", {
  ...
  y <- rbinom(n, 1, prob = 0.5)
  offset <- runif(n)
  fit <- suppressWarnings(glm2_Shrink(y ~ ., family = binomial(link = "logit"), data = data.frame(y, x), offset = offset))
...
test_that("Handling weights argument", {
  ...
  weights <- runif(n, 0, 1)
  fit <- suppressWarnings(glm2_Shrink(y ~ ., family = binomial(link = "logit"), data = data, weights = weights))
  expect_true(is.numeric(fit$coefficients), info = "Coefficients should be numeric")
  expect_true(fit$converged, info = "Model should converge")

  weights <- runif(n, -1, 0)
  expect_error(glm2_Shrink(y ~ ., family = binomial(link = "logit"), data = data, weights = weights),
...
test_that("Offset length mismatch", {
  ...
  offset <- runif(n + 1)
  expect_error(
    glm2_Shrink(y ~ ., family = binomial(link = "logit"), data = data, offset = offset),
...
test_that("Invalid family type", {
  ...
  expect_error(
    glm2_Shrink(y ~ ., family = invalid_family, data = data),
...
test_that("Unrecognized family with NULL family argument and suppresses print output", {
  ...
  expect_error(
    capture.output(suppressMessages(glm2_Shrink(y ~ ., family = NULL, data = data))),
...
test_that("Exclude x and y from the fit object", {
  ...
  fit <- suppressWarnings(glm2_Shrink(y ~ ., family = binomial(link = "logit"), data = data, x = FALSE, y = FALSE))
...
test_that("Include x and y in the fit object", {
  ...
  fit <- suppressWarnings(glm2_Shrink(y ~ ., family = binomial(link = "logit"), data = data, x = TRUE, y = TRUE))
...
test_that("Non-convergence in null deviance fit", {
  ...
  fit <- withCallingHandlers(
    glm2_Shrink(y ~ ., family = binomial(link = "logit"), data = data, control = list(maxit = 1), offset = offset),
    warning = function(w) {
...
test_that("glm2_Shrink handles invalid method argument", {
  set.seed(123)
  ...
  expect_error(
    glm2_Shrink(y ~ x1 + x2, family = poisson(), data = data, method = 123),
    "invalid 'method' argument"
  )
})

test_that("glm2_Shrink handles one-dimensional array response Y correctly", {
  set.seed(123)
  x <- matrix(rnorm(100), ncol = 2)
  colnames(x) <- c("x1", "x2")
  ...
  fit <- suppressWarnings(glm2_Shrink(y ~ x1 + x2, family = poisson(), data = data))
...
test_that("glm2_Shrink processes valid method argument and control options", {
  set.seed(123)
  x <- matrix(rnorm(100), ncol = 2)
  colnames(x) <- c("x1", "x2")
  y <- rpois(50, lambda = exp(x %*% c(0.5, -0.2)))
  ...
  fit <- suppressWarnings(glm2_Shrink(y ~ x1 + x2, family = poisson(),
                                         data = data, model_class = "DSh", method = "glm.fit2_Shrink"))

  expect_s3_class(fit, "glm")
  expect_equal(fit$method, "glm.fit2_Shrink")
})

test_that("glm2_Shrink handles missing data argument correctly", {
  set.seed(123)
  x <- matrix(rnorm(100), ncol = 2)
  colnames(x) <- c("x1", "x2")
  y <- rpois(50, lambda = exp(x %*% c(0.5, -0.2)))
  ...
  fit <- suppressWarnings(glm2_Shrink(formula, family = poisson()))

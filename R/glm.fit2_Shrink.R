...
#'
#' @description \code{glm.fit2_Shrink} is a modified version of \code{glm.fit} in the stats package,
...
#'
#' @usage glm.fit2_Shrink(x, y, weights = rep(1, nobs),
...
#'
#' @details
#' \code{glm.fit2_Shrink} extends \code{glm.fit} by using custom optimization functions to improve the convergence properties of the iteratively reweighted least squares (IRLS) algorithm.
...
#'
#' @return
#' The value returned by \code{glm.fit2_Shrink} has the same structure as the value returned by \code{glm.fit}. It includes the following components:
...
#'
#' @export
glm.fit2_Shrink <- function (x, y, weights = rep(1, nobs),  model_class = c("St", "DSh", "SR", "GSR", "Sh"),
  ...
  if (EMPTY) {
    ...
    if (identical(Sys.getenv("_R_CHECK_PACKAGE_NAME_"), "ShrinkageGLMs")) {
      use_parallel <- FALSE # Disable parallel processing during checks
    }
    if (use_parallel) {
      results <- parallel::mclapply(1:length(fit_functions), function(i) {
        fit_model_parallel(fit_functions[[i]], fit_names[i])
      }, mc.cores = num_cores)
    } else {
      results <- lapply(1:length(fit_functions), function(i) {
        fit_model_parallel(fit_functions[[i]], fit_names[i])
      })
    }
